...
from config import APP_TITLE, DEFAULT_LIMIT, EXPORT_DIR
from src.analyzer import analyze_papers, generate_insights, generate_summary
from src.api_client import demo_papers, fetch_arxiv, fetch_crossref, fetch_openalex
...
def fetch_real_papers(query, source, limit):
    source = normalize_source(source)
    rows = candidate_limit(limit)
    if source == "arxiv":
        raw = fetch_arxiv(query, rows=rows)
        return rank_papers(query, parse_arxiv_response(raw), limit=limit)

    if source == "openalex":
        papers = []
        raw = fetch_openalex(query, rows=rows)
        papers.extend(parse_openalex_response(raw))
        try:
            cited_raw = fetch_openalex(query, rows=rows, sort="cited_by_count:desc")
            papers.extend(parse_openalex_response(cited_raw))
        except Exception:
            pass
        return rank_papers(query, papers, limit=limit)

    raw = fetch_crossref(query, rows=rows)
    ...
    try:
        ...
        pass
    try:
        openalex_raw = fetch_openalex(query, rows=rows)
        papers.extend(parse_openalex_response(openalex_raw))
    except Exception:
        pass
    try:
        openalex_raw = fetch_openalex(query, rows=rows, sort="cited_by_count:desc")
        papers.extend(parse_openalex_response(openalex_raw))
    except Exception:
        pass
    return rank_papers(query, papers, limit=limit)
...
def load_or_fetch(query, source, limit, use_cache=False):
    source = normalize_source(source)
    if use_cache:
    ...
    papers = fetch_real_papers(query, source, limit)
    if not papers:
        cached = cached_query(query, include_demo=False)
        if cached:
            cached = rank_papers(query, cached, limit=limit)
...
def load_papers_safely(query, source, limit, use_cache=False):
    error = None
    try:
        return (*load_or_fetch(query, source, limit, use_cache=use_cache), error)
    except Exception as exc:
        source = normalize_source(source)
        if source in {"arxiv", "openalex"}:
            try:
                papers, data_source, status_message = load_or_fetch(
                    query, "crossref", limit, use_cache=False
                )
...
def search():
    ...
    papers, data_source, status_message, error = load_papers_safely(
        query, source, limit, use_cache=use_cache
    )
...
def analysis():
    ...
    if paper_ids:
        ...
        papers, data_source, status_message, error = load_papers_safely(
            query, source, limit, use_cache=False
        )
        ids_param = paper_ids_for_url(papers)
    else:
        papers = []
