...
- 单篇论文详情与分析：进入论文详情页可看到完整摘要、关键词、本次检索集合中的引用排位、同年论文数对比图，以及基于关键词相似度+引用得分+年份新鲜度的相关推荐。
...
1. 启动应用并访问首页。
2. 输入关键词，例如 `recommendation system`，选择数据源 OpenAlex，数量上限填 200。
3. 点击“调用 API 检索论文”，等待 OpenAlex 通过 cursor 分页拉取结果。
4. 在结果页顶部使用紧凑筛选条：调整排序（引用高→低、年份新→旧）、设置最低引用次数、年份范围、来源或输入关键词二次过滤，实时观察列表变化。
5. 点击任一论文卡片下方“查看详情与分析”，进入 `/paper/<id>` 单篇详情页，查看摘要、关键词、本次检索集合中的引用排位与对比条、相关推荐论文。
6. 返回结果页，点击底部“查看整体分析”CTA，进入 `/analysis` 仪表盘查看年度趋势、关键词分布、引用分布、作者合作网络等 8 张图表与自动洞察。
7. 点击“导出本次 CSV”，下载当前检索结果的 CSV 文件。
